...
const Produtos = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const produtoParaEnviar = {
        ...produto,
        precoUnitario: produto.precoUnitario.toFixed(2), // converte number para string formatada
        dataVencimento: formatForJava(new Date(produto.dataVencimento || new Date())),
        imagem: produto.imagem || null,
      };
    ...
    <produtos>
      <produto>
        <codigo>${produto.ean.toString()}</codigo>
        <nome>${produto.nome}</nome>
        <preco>${produto.precoUnitario.toFixed(2)}</preco>  <!-- também formatado -->
      </produto>
    </produtos>
  `.trim();
                        ...
                        adicionarProduto(
                          {
                            ...
                            ean: typeof produto.ean === 'string'
                              ? parseInt(produto.ean, 10)
                              : produto.ean,
                            quantidade: typeof produto.quantidade === 'string'
